...
export function EventCard({ slug, event }: { slug: string; event: Event }) {
  return (
    <Link
      ...
      className="group"  
    >
      <Card className="overflow-hidden transition-all duration-300 ease-out cursor-pointer hover:scale-[1.02] hover:shadow-2xl border border-zinc-200 dark:border-zinc-800 rounded-xl bg-white dark:bg-zinc-800 p-0">
        ...
        <div className="relative overflow-hidden aspect-[16/9] leading-none">
          <img
            src={event.mainImage}
            alt={event.name}
            className="object-cover w-full h-full transition-transform duration-500 ease-out group-hover:scale-110 display-block"
          />
        </div>

        <div className="px-3 pb-3 pt-0 bg-white dark:bg-zinc-800">
          <div className="flex items-center gap-2 text-xs mb-2">
            <span className="flex items-center gap-1 bg-zinc-100 dark:bg-zinc-700 px-2 py-0.5 rounded-full text-zinc-700 dark:text-zinc-300">
              ...
              <span className="font-medium">{event.tags[0]}</span>
            </span>
            <span className="flex items-center gap-1 bg-zinc-100 dark:bg-zinc-700 px-2 py-0.5 rounded-full text-zinc-700 dark:text-zinc-300">
          ...
          <h3 className="text-lg font-semibold leading-tight text-zinc-900 dark:text-white line-clamp-1">
            {event.name}
          </h3>
        </div>
      </Card>
